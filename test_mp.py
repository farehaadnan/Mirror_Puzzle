...
def build_photobooth_strip(solved_photos, photo_size=300):
    padding = 20
    caption_height = 40
    title_height = 60
    button_height = 70
    ...
    strip_width = photo_size + padding * 4
    cream_width = photo_size + padding * 2
    strip_height = (title_height +
    ...
    cream_x1 = padding
    cream_y1 = title_height - 10
    cream_x2 = cream_x1 + cream_width
    cream_y2 = strip_height - button_height - padding
    cv2.rectangle(strip, (cream_x1, cream_y1), (cream_x2, cream_y2), (225, 235, 240), -1)

    cv2.putText(strip, "YOUR PHOTO STRIP", (padding + 10, 35),
    ...
    for i, photo in enumerate(solved_photos):
        ...
        cv2.rectangle(strip, (px1 - 6, py1 - 6), (px2 + 6, py2 + 6), (255, 255, 255), -1)
        ...
        cv2.putText(strip, caption, (text_x, py2 + 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (80, 80, 80), 1)

        y_cursor = py2 + caption_height + padding

    btn_y1 = strip_height - button_height - padding // 2
    btn_y2 = strip_height - padding // 2
    btn_gap = 15
    btn_w = (cream_width - btn_gap) // 2

    download_x1 = cream_x1
    download_x2 = download_x1 + btn_w
    close_x1 = download_x2 + btn_gap
    close_x2 = close_x1 + btn_w

    cv2.rectangle(strip, (download_x1, btn_y1), (download_x2, btn_y2), (0, 190, 255), -1)
    cv2.putText(strip, "DOWNLOAD", (download_x1 + 25, btn_y1 + 45),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (20, 20, 20), 2)

    cv2.rectangle(strip, (close_x1, btn_y1), (close_x2, btn_y2), (60, 60, 60), 2)
    cv2.putText(strip, "CLOSE", (close_x1 + 45, btn_y1 + 45),
...
def show_final_strip_and_wait(solved_photos):
    strip = build_photobooth_strip(solved_photos)

    cv2.imshow("Photo Strip", strip)
    print("Press D to download/save, C to close without saving.")
